...
| **Comments / Guestbook** | **Anonymous nickname + password** (Tistory-style): anyone posts with a nickname + password, and that password edits/deletes their own entry. Owner moderates (delete any). Post _writing_ stays owner-only. |
| **Deploy**               | Cloudflare (Workers + D1 + R2). Migrate to AWS _later_ as an infra-learning project.                                                                                                                        |
...
- [x] Prettier config — done 2026-08-05. `prettier.config.mjs`(+`.prettierignore`), `pnpm format` / `format:check`, CI에 검사 추가. `eslint-config-prettier`를 eslint 설정 끝에 넣어 서식 규칙은 Prettier 한 곳에서만 정하게 했다
  - `prettier-plugin-tailwindcss`가 설치만 되어 있고 **설정이 없어 실제로는 돌지 않았다** — 등록하니 Tailwind 클래스 정렬이 처음 적용됐다. Tailwind 4는 설정 파일이 없어서 `tailwindStylesheet`로 CSS 진입점을 알려줘야 한다
    > 플러그인이 코드를 깨뜨린 사례: `` `…hover:bg-surface${mono ? " font-mono" : ""}` `` 의 앞 공백을 지워 `hover:bg-surfacefont-mono`로 만들었다. **클래스를 템플릿 리터럴로 이어붙이지 말고 배열에 담아 `join(" ")`** 할 것.
    > format-on-save는 에디터 설정이라 저장소 범위 밖 — VS Code라면 `editor.formatOnSave` + Prettier 확장.

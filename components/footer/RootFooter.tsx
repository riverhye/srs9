...
// 소셜 단색 글리프(simple-icons, 24 viewBox, 단일 path). fill=currentColor라
// muted 토큰 색을 입고 hover 시 foreground로. email은 gmail 글리프를 메일 표식으로.
...
export function RootFooter() {
  ...
  const links = [
    github && { name: "github", label: "GitHub", href: github, external: true },
    email && {
      name: "email",
      label: "Email",
      href: `mailto:${email}`,
            ...
            {links.map((l) => (
              ...
              >
                <svg
                  ...
                  className="size-5"
                >
                  <path d={ICON_PATHS[l.name]} />
                </svg>
              </a>
            ))}
          </div>
        )}
        <p className="mt-4 text-xs text-muted">© {year} srs9</p>
      </div>
    </footer>
  );
}
